...
import 'package:stepper/my_home_page.dart';
...
class BaseStepperState extends State<BaseStepper> {
  ...
  List<Widget> _buildSteps() {
    // print("Printed From Base: ${widget.completedTasks}");
    return List.generate(
      widget.children!.length,
      (index) {
        // print("Checking true flase from base Stepper $index ${widget.completedTasks![index.toString()] == 0 ? false : true}");
        return widget.direction == Axis.horizontal
            ? Row(
                children: <Widget>[
  ...
  Widget _customizedIndicator(int index) {
    return BaseIndicator(
      ...
      completedColor: widget.completeStepColor,
  ...
  void _goToNextStep() {
    if (_selectedIndex < widget.children!.length - 1 &&
      ...
      setState(() {
        // print("Base Stepper Selected Index : $_selectedIndex ");
        widget.isCompleted!(_selectedIndex);
        _selectedIndex++;
        if (widget.onStepReached != null) {
          widget.onStepReached!(_selectedIndex);
        }
